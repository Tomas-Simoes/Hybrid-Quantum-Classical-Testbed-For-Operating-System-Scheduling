...
class RunConfig(BaseModel):
    ...
    def clamp_public_values(self) -> RunConfig:
        ...
        self.total_weight = _clamp_float(float(self.total_weight), 0.001, 100.0)

        if self.weights:
            weights = [max(0.0, float(weight)) for weight in self.weights[: self.num_processes]]
        ...
        self.penalty = _clamp_float(float(self.penalty), 0.001, 1_000.0)
        if self.target_load is not None:
            self.target_load = max(0.0, float(self.target_load))
        ...
        self.learning_rate = _clamp_float(float(self.learning_rate), 0.0001, 1.0)
        self.top_k = _clamp_int(int(self.top_k), 1, settings.public_max_top_k)
        self.init_gamma = _clamp_float(float(self.init_gamma), 0.0, math.tau)
        self.init_beta = _clamp_float(float(self.init_beta), 0.0, math.tau)
        ...
        self.qubit_max = max(min_qubit_max, int(requested_qubit_max))

        self.io_alpha = _clamp_float(float(self.io_alpha), 0.0, 1.0)
        self.affinity_alpha = _clamp_float(float(self.affinity_alpha), 0.0, 1.0)
        self.homogeneity_threshold = _clamp_float(float(self.homogeneity_threshold), 0.0, 10.0)
        self.zscore_threshold = _clamp_float(float(self.zscore_threshold), 0.0, 10.0)

        self.min_rss = max(0.0, float(self.min_rss))
        self.min_cpu = max(0.0, float(self.min_cpu))
