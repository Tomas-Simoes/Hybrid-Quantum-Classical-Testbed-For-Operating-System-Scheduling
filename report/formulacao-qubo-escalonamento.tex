...
No entanto, a decisão de atribuição de processos a núcleos é fundamentalmente um problema de partição combinatorial. Dado um conjunto de $N$ processos com cargas $w_i$ e $K$ núcleos, o número de atribuições possíveis cresce como $K^N$, tornando a procura exaustiva para instâncias de grande escala~\cite{Kochenberger2014QUBO,Glover2019QUBO}.

Este projeto adota uma formulação simplificada deste problema: cada entidade possui
...
Um problema \acs{QUBO}~\cite{Kochenberger2014QUBO,Glover2019QUBO} define uma função
de energia quadrática sobre variáveis binárias $x_i \in \{0, 1\}$:

\begin{equation}
    E(\mathbf{x}) = \mathbf{x}^\top Q\, \mathbf{x} = \sum_{i} \sum_{j} Q_{ij} \cdot x_i \cdot x_j,
    \label{eq:energia-qubo}
\end{equation}
\listeq{Energia QUBO}

onde $Q \in \mathbb{R}^{n \times n}$ é a matriz de coeficientes e $\mathbf{x} \in \{0,1\}^n$.
O problema consiste em encontrar:

\begin{equation}
    \mathbf{x}^* = \min_{\mathbf{x} \in \{0,1\}^n} E(\mathbf{x}).
\end{equation}
\listeq{Problema de otimização QUBO}
...
onde $L_j$ é a carga total atribuída ao núcleo $j$ e
...
é a carga média alvo entre os $K$ núcleos.

Para obter a formulação \acs{QUBO}, este termo é expandido e o resultado é codificado na matriz $Q$, de modo que o \textit{solver}
minimize a energia $E(\mathbf{x})$.
...
onde $\phi_j$ representa a carga já comprometida nesse núcleo, relevante para a
decomposição iterativa descrita na Secção~\ref{sec:extracao-subqubo}. Na primeira iteração,
ou num \acs{QUBO} sem carga fixa, tem-se
$\phi_j = 0$.
...
Somando esta expansão sobre todos os núcleos $j$, o desvio quadrático total iguala-se,
termo a termo, à energia \acs{QUBO} da Equação~\ref{eq:energia-qubo}:
\begin{equation}
    \sum_{j=0}^{K-1}(L_j - L_{avg})^2 = \mathbf{x}^\top Q\, \mathbf{x} + C,
\end{equation}
\listeq{Equivalência entre o desvio de carga e a energia QUBO}
onde $C = \sum_{j=0}^{K-1}(\phi_j - L_{avg})^2$ é uma constante independente de $\mathbf{x}$,
e as entradas de $Q$ correspondem diretamente aos coeficientes obtidos na expansão: $d_{ij}$
na diagonal e $w_iw_k$ nas entradas fora da diagonal, para processos $i,k$ atribuídos ao
mesmo núcleo $j$. Minimizar $E(\mathbf{x})$ é, portanto, exatamente equivalente a minimizar
o desvio de carga original.
...
exatamente um núcleo. Para isso adiciona-se à matriz $Q$ a penalidade de atribuição única:

\begin{equation}
    P \sum_{i=0}^{N-1}\left(\sum_{j=0}^{K-1}x_{ij} - 1\right)^2.
\end{equation}
\listeq{Penalidade de Atribuição Única}
...
Combinando o objetivo de balanceamento e a penalidade de atribuição única, cada entrada da matriz
...
corresponde à penalidade de atribuição única entre núcleos diferentes do mesmo processo. A última
...
Antes de adicionar a penalidade de atribuição única, os termos diagonais do balanceamento são:
\begin{equation}
\begin{aligned}
...
Com $P=1.0$, a penalidade subtrai $1.0$ a cada termo diagonal e adiciona $1.0$
...
Para verificar o efeito dos termos de co-localização, comparem-se duas soluções
viáveis, com $\mathbf{x}^\top = (x_{00}, x_{01}, x_{10}, x_{11})$:
A, com os processos distribuídos ($\mathbf{x}_A^\top = (1,0,0,1)$), e B, com ambos
os processos no núcleo $0$ ($\mathbf{x}_B^\top = (1,0,1,0)$). Usando $E(\mathbf{x}) = \mathbf{x}^\top Q\mathbf{x}$ \eqref{eq:energia-qubo}:
\begin{equation}
\begin{aligned}
E_A &= Q_{0,0} + Q_{3,3} = -1.15 + (-1.15) = \mathbf{-2.30},\\
E_B &= Q_{0,0} + Q_{2,2} + Q_{0,2} + Q_{2,0} = -1.15+(-1.15)+0.15+0.15 = \mathbf{-2.00}.
\end{aligned}
\end{equation}
...
com \acs{QAOA}~\cite{Farhi2014QAOA}.
